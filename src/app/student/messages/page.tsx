...
export default function StudentMessagesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await fetchAPI('/api/student/messages');
        setMessages(data);
      } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-8 h-full min-h-[calc(100vh-160px)]">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="font-display text-[32px] md:text-[40px] font-bold text-primary mb-2">Messages</h1>
          <p className="text-on-surface-variant text-[18px]">Direct communication with your tutors and administration.</p>
        </div>
      ...
      <div className="bg-surface-container-lowest rounded-xl shadow-ambient border border-outline-variant/30 flex-1 flex overflow-hidden">
        ...
        <div className={`w-full md:w-2/3 flex flex-col ${!selectedId ? 'hidden md:flex' : 'flex'}`}>
          {selectedMessage ? (
            <>
              <div className="p-4 md:p-6 border-b border-outline-variant/30 flex items-center gap-4 bg-surface-container-lowest">
                <button 
