...
export default async function StudentSchedulePage() {
  const schedule = await fetchAPI('/api/student/schedule');

  // Group by date (simplified for mock data)
  const groupedSchedule = schedule.reduce((acc: any, curr: any) => {
    const date = new Date(curr.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
                  ...
                  <div key={session.id} className="bg-surface-container-lowest rounded-xl p-6 shadow-ambient border border-outline-variant/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 group hover:border-primary/30 transition-colors">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <h3 className="font-headline text-[20px] font-bold text-on-surface mb-1">{session.courseTitle}</h3>
                        <p className="text-on-surface-variant flex items-center gap-2 text-sm">
                          ...
                          <span className="sm:hidden">{new Date(session.date).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</span>
                        </p>
                      </div>
                    </div>
